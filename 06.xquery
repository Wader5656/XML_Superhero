...
declare function local:getmax($superheroes, $publishers, $num-by-pub) {
    fn:max(for $publisher in $publishers
    return
        $superheroes?*[?biography?publisher = $publisher] => count())

};

declare function local:getmin($superheroes, $publishers, $num-by-pub) {
    fn:min(for $publisher in $publishers
    return
        $superheroes?*[?biography?publisher = $publisher] => count())
};
...
return
    map:entry($publisher, $superheroes?*[?biography?publisher = $publisher] => count())
)
