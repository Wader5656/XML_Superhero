...
declare function local:maximumheight($superheroes) as xs:double{
    fn:max(for $s in $superheroes?* where not(substring-before($s?appearance?height?1,"'") eq "") return number(substring-before($s?appearance?height?1,"'")) )
};


declare function local:maximumweight($superheroes) as xs:double{
    fn:max(for $s in $superheroes?* where not(contains(substring-before($s?appearance?weight?1,' '),'-')) return number(substring-before($s?appearance?weight?1,' ')))
};
